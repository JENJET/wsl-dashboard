...
pub fn add_port_proxy_elevated(
    listen_addr: &str,
    listen_port: u16,
    connect_addr: &str,
    connect_port: u16,
    _enable_firewall: bool,
    _distro_name: &str,
) -> Result<(), String> {
    info!(
        "Requesting elevation for netsh add portproxy (Direct: {}:{} -> {}:{})",
        listen_addr, listen_port, connect_addr, connect_port
    );
...
pub fn add_port_proxy_and_firewall_elevated(
    ...
    if enable_firewall {
        ...
        commands.push(super::firewall::get_add_rule_cmd_netsh(
            &rule_name,
            listen_port,
            listen_addr,
        ));
...
pub fn add_firewall_rule_elevated(
    listen_addr: &str,
    listen_port: u16,
    distro_name: &str,
) -> Result<(), String> {
...
pub fn delete_port_proxy_elevated(
    listen_addr: &str,
    listen_port: u16,
    _distro_name: &str,
) -> Result<(), String> {
    info!(
        "Requesting elevation to delete portproxy (Direct): {}:{}",
...
pub fn delete_port_proxy_and_firewall_elevated(
    listen_addr: &str,
    listen_port: u16,
    distro_name: &str,
) -> Result<(), String> {
...
pub fn apply_port_proxies_elevated(
    rules_with_ips: Vec<(PortProxyRule, String)>,
) -> Result<(), String> {
    if rules_with_ips.is_empty() {
        return Ok(());
    }

    info!(
        "Batch applying {} port proxy rules via elevation",
        rules_with_ips.len()
    );
...
pub fn delete_port_proxies_elevated(rules: Vec<PortProxyRule>) -> Result<(), String> {
    ...
    info!(
        "Batch deleting {} port proxy rules via elevation",
        rules.len()
    );
    let mut all_cmd_parts = Vec::new();
    for rule in rules {
        all_cmd_parts.push(format!(
            "netsh interface portproxy delete v4tov4 listenaddress={} listenport={}",
...
pub fn sync_port_proxies(
    ...
    let target_ip = if is_running {
        match get_distro_ip(distro_name, None) {
            Ok(ip) => Some(ip),
            Err(e) => {
                error!(
                    "Failed to obtain IP for running distro {}: {}",
                    distro_name, e
                );
                None
            }
    ...
    info!(
        "Synchronization completed for {}. {} rules processed.",
        distro_name,
        results.len()
    );
    Ok(results)
