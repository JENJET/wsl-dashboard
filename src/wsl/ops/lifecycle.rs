...
pub async fn start_distro(
    ...
    let probe_result = executor
        .execute_command(&["-d", distro_name, "--", "sh", "-c", "echo 'starting'"])
        .await;

    if !probe_result.success {
        warn!(
            "Failed to start WSL distro {}: {:?}",
            distro_name, probe_result.error
        );
        return probe_result;
    }
    ...
    task::spawn_blocking(move || {
        info!(
            "Starting background keep-alive process for WSL distro: {}",
            distro_name_owned
        );
        ...
        match cmd.spawn() {
            Ok(_child) => {
                info!(
                    "Successfully spawned keep-alive process for {}",
                    distro_name_owned
                );
            ...
            Err(e) => {
                error!(
                    "Failed to spawn keep-alive process for {}: {}",
                    distro_name_owned, e
                );
            }
        }
    ...
    WslCommandResult::success(
        format!(
            "Distro '{}' started and keep-alive process initiated",
            distro_name
        ),
        None,
    )
...
pub async fn stop_distro(
    ...
    executor
        .execute_command(&["--terminate", distro_name])
        .await
}
...
pub async fn delete_distro(
    ...
    let all_distros_reg =
        task::spawn_blocking(|| crate::utils::registry::get_wsl_distros_from_reg())
            .await
            .unwrap_or_default();
    ...
    if let Some(info) = target_distro_info {
        ...
        if !pfn.is_empty() {
            ...
            let pfn_count = all_distros_reg
                .iter()
                .filter(|d| &d.package_family_name == pfn)
                .count();
            if pfn_count == 1 {
                pfn_to_remove = Some(pfn.clone());
                info!(
                    "Distribution '{}' is associated with package '{}' and is the only instance using it.",
                    distro_name, pfn
                );
            } else {
                info!(
                    "Distribution '{}' is associated with package '{}', but {} other instances still use this launcher.",
                    distro_name,
                    pfn,
                    pfn_count - 1
                );
    ...
    info!(
        "Cleaning up configurations for '{}' before unregistering",
        distro_name
    );
    ...
    debug!(
        "Starting parallel cleanup tasks for '{}' with 15s timeout...",
        distro_name
    );
    ...
    if cleanup_result.is_err() {
        warn!(
            "Parallel cleanup tasks for '{}' timed out after 15s. Proceeding.",
            distro_name
        );
    }

    debug!(
        "Finished parallel cleanup tasks attempt for '{}'",
        distro_name
    );
    ...
    match config_res {
        Ok(Err(e)) => warn!(
            "Failed to remove instance config for '{}': {}",
            distro_name, e
        ),
    ...
    if !exists_in_reg {
        debug!(
            "Distribution '{}' not found in registry, skipping WSL terminate/unregister.",
            distro_name
        );
    } else {
        ...
        debug!(
            "Executing WSL command: wsl --unregister {} (20s timeout)...",
            distro_name
        );
        ...
        if !result.success {
            warn!(
                "Failed to unregister WSL distro '{}': {:?}",
                distro_name, result.error
            );
            return result;
        }
    ...
    if let Some(pfn) = pfn_to_remove {
        info!(
            "Initiating launcher cleanup for PackageFamilyName: {} (Background)",
            pfn
        );
        ...
        tokio::spawn(async move {
            ...
            let cleanup_res = tokio::time::timeout(std::time::Duration::from_secs(15), async {
    ...
    WslCommandResult::success(
        format!(
            "Distro '{}' deleted and launcher cleanup initiated",
            distro_name
        ),
        None,
    )
